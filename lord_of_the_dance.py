...
def train(args):
    ...
    run=wandb.init(reinit=True,config=config,project="appliedDL",
                   name=("gen-"+config["gen_type"]+" data- "+config["data_dir"]+"_"+str(config["class_dim"])+"_"+str(config["content_dim"])))
    lord = Lord(config)
    lord.train_latent(
        imgs=imgs,
...
def train_encoders(args):
    ...
    data = np.load(assets.get_preprocess_file_path(args.data_name))
    imgs = data['imgs'].astype(np.float32)
    run=wandb.init(reinit=True,project="appliedDL_encoder")
    lord = Lord()
